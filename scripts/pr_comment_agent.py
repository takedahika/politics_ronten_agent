...
def main():
    ...
    # 2. Identify target files (Markdown AND topic.yaml files modified in this PR)
    pr_files = pr.get_files()
    target_files = []
    for f in pr_files:
        if f.filename.startswith("topics/") and (
            f.filename.endswith(".md") or f.filename.endswith("topic.yaml")
        ):
            target_files.append(f.filename)

    if not target_files:
        pr.as_issue().create_comment(
            "申し訳ありません。このPRには修正対象となるトピックのファイルが含まれていないため、自動修正を実行できません。"
        )
        sys.exit(0)
    ...
    # 4. Prepare files content (both .md and .yaml)
    files_content = {}
    ...
    for filename, content in files_content.items():
        ...
        lang = "yaml" if ext == "yaml" else "markdown"
        files_section += f"\n--- {filename} ---\n```{lang}\n{content}\n```\n"

    prompt = f"""あなたは客観的で中立な「論点の現在地」の編集AIです。
ユーザーからPull Requestのコメントで記事・トピック設定の修正指示を受けました。
...
- Markdownファイル（.md）は記事の文章・構成を修正してください。
- topic.yaml は、タイトル（title）、概要（description）、キーワード（keywords）、国会検索キーワード（ndl_keyword）などを修正できます。
  ただし、id・slug・status・created_atは変更しないでください。

【出力フォーマット】
必ず以下のJSONフォーマットで出力してください。変更が不要なファイルは含めなくて構いません。装飾やJSON以外のテキストは含めないでください。
{{
  "ファイルのパス": "修正後の完全なテキスト（.mdならMarkdown、.yamlならYAML形式）",
  "ファイルのパス2": "修正後の完全なテキスト"
}}

【絶対ルール】
1. 「AIがまとめました」「私が提案します」などのAI自身についてのメタな言及や挨拶は一切含めないこと。
2. 文章は「だ・である調」で知的なトーンを維持すること。
3. 高校三年生（18歳）が読んでスムーズに理解できるよう、官公庁や法律の特有の難解な熟語（例：「乏しい」「属し」「不均衡」など）は使用を禁止し、平易な言葉に翻訳すること。
4. Markdownのフォーマット（見出し、リスト、リンクなど）は絶対に崩さないこと。
5. YAMLのフォーマットを壊さないこと。
"""
    ...
    for filename, new_content in updated_files.items():
        ...
        # YAMLファイルの場合、id/slug/status/created_at を保護する
        if filename.endswith("topic.yaml"):
            try:
                original_yaml = yaml.safe_load(files_content[filename])
                new_yaml = yaml.safe_load(new_content)
                # 保護するフィールドを元の値に戻す
                for protected_key in ["id", "slug", "status", "created_at"]:
                    if protected_key in original_yaml:
                        new_yaml[protected_key] = original_yaml[protected_key]
                new_content = yaml.dump(new_yaml, allow_unicode=True, default_flow_style=False, sort_keys=False)
            except Exception as e:
    ...
    try:
        ...
        pr.as_issue().create_comment(
            f"修正は作成できましたが、コミット・プッシュ中にエラーが発生しました。\n`{e}`"
        )
        sys.exit(1)
